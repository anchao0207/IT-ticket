...
export default function TimeLogsPage() {
    ...
    useEffect(() => {
        ...
        const fmt = (d: number) => `${year}-${(month + 1).toString().padStart(2, '0')}-${d.toString().padStart(2, '0')}`;
    ...
    const getPeriodLabel = () => {
        ...
        return `${monthName} (${viewPart === 'first' ? '1st - 15th' : '16th - End'})`;
    };
    ...
    useEffect(() => {
        ...
        fetch(`/api/time-logs?technicianId=${selectedAdminId}&from=${dateRange.start}&to=${dateRange.end}`)
            .then(res => res.json())
            .then((data: TimeLog[]) => {
                setDbLogs(data);
                ...
                const merged = days.map(date => {
                    ...
                    const existing = data.find(l => new Date(l.date).toISOString().slice(0, 10) === dateStr);

                    const formatTime = (iso?: string) => iso ? new Date(iso).toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit' }) : '';
                    ...
                    return {
                        ...
                        mileage: existing?.mileage !== undefined && existing.mileage !== null ? existing.mileage.toString() : '0.0',
                    };
                });
    ...
    // 3. UI Helpers
    const handleInputChange = (dateStr: string, field: keyof TimeLog, value: string) => {
    ...
    const totalPeriodHours = editableLogs.reduce((acc, log) => acc + calculateDailyTotal(log), 0);
    ...
    const handleSave = async () => {
        ...
        const promises = editableLogs.map(async (log) => {
            ...
            const toISO = (timeStr?: string) => timeStr ? `${log.date}T${timeStr}:00` : null;
    ...
    const isOwnLog = currentUser && selectedAdminId && currentUser.id.toString() === selectedAdminId.toString();

    return (
        <div className="min-h-screen bg-gray-50 p-6 dark:bg-gray-900">
            <div className="mx-auto max-w-6xl rounded-lg bg-white p-6 shadow dark:bg-gray-800">
                <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Time Logs</h1>
                    ...
                    <table className="w-full text-left text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            {editableLogs.map(log => {
                                // Display Date
                                const displayDate = new Date(log.date + 'T00:00:00').toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' });

                                return (
                                    <tr key={log.date} className={`border-b ${isEditing ? 'bg-blue-50/50' : 'bg-white'} dark:border-gray-700 dark:bg-gray-800`}>
                                        ...
                                        <td className="px-6 py-4">
                                            <input
                                                ...
                                                value={log.timeIn || ''}
                                                onChange={(e) => handleInputChange(log.date, 'timeIn', e.target.value)}
                                                disabled={!isEditing}
                                            />
                                        </td>
                                        <td className="px-6 py-4">
                                            <input
                                                ...
                                                onChange={(e) => handleInputChange(log.date, 'lunchStart', e.target.value)}
                                                disabled={!isEditing}
                                            />
                                        </td>
                                        <td className="px-6 py-4">
                                            <input
                                                ...
                                                className="w-full rounded border p-1 disabled:bg-transparent disabled:border-transparent dark:bg-gray-700"
                                                value={log.lunchEnd || ''}
                                                onChange={(e) => handleInputChange(log.date, 'lunchEnd', e.target.value)}
                                                disabled={!isEditing}
                                            />
                                        </td>
                                        <td className="px-6 py-4">
                                            <input
                                                ...
                                                className="w-full rounded border p-1 disabled:bg-transparent disabled:border-transparent dark:bg-gray-700"
                                                value={log.timeOut || ''}
                                                onChange={(e) => handleInputChange(log.date, 'timeOut', e.target.value)}
                                                disabled={!isEditing}
                                            />
                                        </td>
                                        <td className="px-6 py-4">
                                            <input
                                                ...
                                                className="w-full rounded border p-1 disabled:bg-transparent disabled:border-transparent dark:bg-gray-700"
                                                value={log.mileage || ''}
                                                onChange={(e) => handleInputChange(log.date, 'mileage', e.target.value)}
                                                disabled={!isEditing}
                                            />
                                        </td>
                            ...
                            <tr className="bg-gray-100 font-bold dark:bg-gray-700">
                                <td colSpan={6} className="px-6 py-4 text-right">Period Total:</td>
